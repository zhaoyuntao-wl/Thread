...
| `buildStatusCard(store, opts)` | status-card | Per-turn status card (merged views + budget tiers). |
...
`StructuredWriteOptions`, `Episode`, `LineageNeighbor`, `SearchHit`, `ThreadPaths`.
