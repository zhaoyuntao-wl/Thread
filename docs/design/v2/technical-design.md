...
> **2026-08-14 更正（Qoder 澄清，已核实）**：结构化查询路径**已实现**——`queryEvents`（query.ts:102，时序/kind 过滤/计数/排序完整）＋ MCP 工具 schema 内置结构化参数（`kind`/`since`/`until`/`order`/`count_only`，server.ts 内聚路由）＋ 单测（query.test.ts）均已落地。剩余缺口 = 协作场景便捷封装（如"对方最近做了什么"一键查询）＋ 状态卡按上下文注入检索提醒（提醒治理见不变量 #11）。
...
11. **主动权在 Thread（2026-08-14 用户定，顶层原则）**：模型不可控，**不把希望放在模型上**——Thread 与模型的交互口越少越好，模型不需要理解记忆系统的结构；状态卡主动注入提醒触发查询（§3.3 设计方向），模型只需记住一条行为契约：**"需要啥就来问，别自己瞎猜"**。任何新能力先问"能否收进现有接口 / 能否由状态卡提醒驱动"，禁止靠暴露更多接口让模型自觉识别。
    ...
    - **注入安全原则（2026-08-15 外部借鉴④，dsh-routing-suite router amnesia 教训）**：Thread 的每轮注入一律以**追加 user message** 形式落地（pre-step `agent.inject` / Qoder additionalContext），**禁止改写、删除、替换底座任何 section**（persona/plan 等）——routing 套件曾整段替换 section 导致模型丢 plan 边界、重复探索仓库（amnesia 实测）；追加式注入天然免疫该病，原则写死防回归。
