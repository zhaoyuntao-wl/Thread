...
| 7 | **成本节省**：长会话上下文不再随历史线性膨胀，同任务总 token 下降（目标架构） | 每轮上下文 = 状态卡 + 按需检索片段，替代全量历史重放 | eval-compare.mjs 同任务前后 N 次中位数对比（待数据积累）。**成立条件（备注）**：检索频次受控时节省成立；若任务每轮都需大量检索片段，检索 token 会侵蚀节省——需度量验证后公告。现状注记：Qoder 薄适配模式下状态卡为净增成本（v1 §MVP 状态注记），节省仅在完全接管形态（t-pi/t-omp）下兑现 |
...
6. **压缩派弱点低成本补强包**（2026-08-14 提出，复用现有 hook 位 + 结构化表）：① PreCompact 注入状态卡进摘要上下文——摘要天然含决策清单，直接提升底座压缩质量（**待 spike**：PreCompact 的 hookSpecificOutput 是否进摘要 LLM prompt；PostCompact 注入实测不被采纳，两者行为可能不同）；② PreToolUse 查反馈表命中教训 → deny 或警告——教训从提示升级为强制；③ 状态卡检测上下文压力，超阈值附压缩建议；④ Stop 时从结构化表拼"会话交接卡"写 `.thread/handoff.md`，新会话开场读取。优先级 ① 最高（半小时级 spike），②③④ 随⑤实施
