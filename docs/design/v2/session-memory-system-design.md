...
> 代码级设计与约束（模块边界 / 数据模型 v2 / 核心接口契约 / 不变量 9 条 / 适配器契约 / 验证体系 / 存储治理）见 [technical-design.md](./technical-design.md)。业务设计（端到端流程 / 输入输出契约 / 用户可见行为 / 操作约束 / 配置面）见 [business-design.md](./business-design.md)。
